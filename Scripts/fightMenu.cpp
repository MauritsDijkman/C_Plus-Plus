...
FightMenu::FightMenu(SceneHandler& sceneHandler, SpriteObject& background, Character& player)
{
	sceneHandler.addScene(this->scene);
	SetupScene(sceneHandler, background, player);
}

FightMenu::~FightMenu() {}


void FightMenu::SetupScene(SceneHandler& sceneHandler, SpriteObject& background, Character& player) {
	// Positions
	...
	// Setting values and generating
	SetValuesFightScene(this->scene, playerTurn, score, proceedButton, attackButton, healButton, specialDescriptionText);
	SetPlayer(player, playerName, playerHP, playerAttack, playerDefense, playerSprites);
	GenerateRandomEnemy(enemySelected, enemyCharacters, enemySprites, enemyName, enemyHP, enemyAttack, enemyDefense, descriptionText, previousEnemy);
}


void FightMenu::Setup_AttackButton(sf::Music& fightMusic, Character& player) {
	attackButton.setButtonAction([=, &fightMusic, &player]() {
...
void FightMenu::Setup_HealButton(sf::Sound& healSound, Character& player) {
	healButton.setButtonAction([=, &healSound, &player]() {
		if (playerTurn) {
...
void FightMenu::Setup_ProceedButton(bool& youngling, bool& apprentice, bool& master, sf::Music& fightMusic, Character& player) {
	proceedButton.setButtonAction([=, &fightMusic, &player]() {
		...
		if (enemyCharacters[enemySelected].getHP() > 0) {
			EnemyTurn(youngling, apprentice, master, fightMusic, player);
		}
...
void FightMenu::Setup_BackButton(SceneHandler& sceneHandler, Scene& previousScene, sf::Sound& backSound, sf::Music& menuMusic, sf::Music& fightMusic, Character& player) {
	backButton.setButtonAction([=, &sceneHandler, &previousScene, &backSound, &menuMusic, &fightMusic, &player]() {
		RemoveImages(player, playerSprites, enemySprites, enemySelected);
...
void FightMenu::SetValuesFightScene(Scene& fightScene, bool& playerTurn, int& score, Button& proceedButton, Button& attackButton, Button& healButton, TextObject& specialDescriptionText) {
...
void FightMenu::SetPlayer(Character& player, TextObject& playerName, TextObject& playerHP, TextObject& playerAttack, TextObject& playerDefense, SpriteObject playerSprites[]) {
	...
	if (player.getName() == "character1") {
		playerSprites[0].setPosition(sf::Vector2f(20.0f, 36.0f));
		this->scene.addGameObject(playerSprites[0]);
	}
	else if (player.getName() == "character2") {
		playerSprites[1].setPosition(sf::Vector2f(20.0f, 36.0f));
		this->scene.addGameObject(playerSprites[1]);
	}
	else if (player.getName() == "character3") {
		playerSprites[2].setPosition(sf::Vector2f(20.0f, 36.0f));
		this->scene.addGameObject(playerSprites[2]);
	}
}

void FightMenu::GenerateRandomEnemy(int& enemySelected, Character enemyCharacters[], SpriteObject enemySprites[], TextObject& enemyName, TextObject& enemyHP, TextObject& enemyAttack, TextObject& enemyDefense, TextObject& descriptionText, std::string previousEnemy) {
...
void FightMenu::Heal(std::string healWho, Character& player, Character enemyCharacters[], int enemySelected, int& healDone, TextObject& playerHP, TextObject& enemyHP, TextObject& descriptionText, bool& playerTurn, sf::Sound& healSound) {
...
void FightMenu::DoNothing(TextObject& descriptionText, Character enemyCharacters[], int enemySelected, bool& playerTurn, sf::Sound& donothingSound) {
...
void FightMenu::EnemyTurn(bool youngling, bool apprentice, bool master, sf::Music& fightMusic, Character& player) {
...
void FightMenu::RemoveImages(Character& player, SpriteObject playerSprites[], SpriteObject enemySprites[], int enemySelected) {
